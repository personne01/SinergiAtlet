...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await register(email, password, fullName, 'talent');
      navigate('/market', { replace: true });
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-[10px] text-white/40 font-mono mt-1">Daftar sebagai Talent</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-[#111111] border border-white/10 rounded-2xl p-6 space-y-4">
          ...
          <div>
            <label className="text-[9px] font-bold text-white/60 uppercase tracking-wider mb-1.5 block">Nama Lengkap</label>
            <input
        ...
        <p className="text-center mt-4 text-[9px] text-white/30">
          Sudah punya akun?{' '}
          <Link to="/login" className="text-[#D1FF00] font-bold hover:underline">Masuk</Link>
        </p>

        <div className="mt-6 p-3 bg-white/5 border border-white/10 rounded-lg text-center">
          <p className="text-[7px] text-white/20 font-mono">
            Klub / Pencari Bakat? Hubungi admin untuk pendaftaran.
          </p>
        </div>
      </motion.div>
    </div>
